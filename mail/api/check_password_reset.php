...
if (!$email || $code === '') send_error('Некорректные параметры.', 'invalid_params', 400);
...
// TTL 10 минут
if (isset($data['created_at']) && time() - intval($data['created_at']) > 600) {
    send_error('Срок действия кода истёк.', 'code_expired', 400);
}
